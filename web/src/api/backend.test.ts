...
import { BackendClient, BackendConnectionError } from './backend.js';
...
describe('BackendClient', () => {
  it('gets ordered persisted AI decisions from the game history endpoint', async () => {
    ...
    const client = new BackendClient('http://127.0.0.1:3001', (input) => {
      const url =
        typeof input === 'string'
          ? input
  ...
  it('invokes the default browser fetch with the global receiver', async () => {
    ...
    try {
      const client = new BackendClient('http://127.0.0.1:3001');
  ...
  it('does not swallow cancellation while reading a problem body', async () => {
    ...
    const client = new BackendClient('http://127.0.0.1:3001', (_input, init) =>
      Promise.resolve(
        new Response(
          new ReadableStream({
            start(stream) {
  ...
  it('uses every approved backend route and HTTP method', async () => {
    const settings = {
    ...
    const client = new BackendClient('http://127.0.0.1:3001/', fetcher);
