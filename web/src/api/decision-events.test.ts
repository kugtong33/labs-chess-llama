...
describe('decision event adapter', () => {
  ...
  it('deduplicates events, bounds memory, and tracks reconnection state', async () => {
    ...
    const { result, unmount } = renderHook(() =>
      useDecisionEvents('http://backend/api/demo/events?gameId=game', {
  ...
  it('falls back cleanly when tracing is disabled', () => {
    ...
    const { result } = renderHook(() =>
      useDecisionEvents('http://backend/events', {
        enabled: false,
        createEventSource,
      }),
    );
  ...
  it('clears traces and starts a bounded dedupe window when the game URL changes', async () => {
    ...
    const { result, rerender } = renderHook(
      ...
      { initialProps: { url: 'http://backend/events?gameId=first' } },
    );
    await waitFor(() =>
    ...
    rerender({ url: 'http://backend/events?gameId=second' });
