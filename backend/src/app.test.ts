...
import { buildApp, type BackendDependencies } from './app.js';
...
function buildTestApp(
  ...
  const dependencies: BackendDependencies = {
    service: service as unknown as BackendDependencies['service'],
    settings: {
      get: () => settings,
...
describe('Fastify backend API', () => {
  ...
  it('streams filtered buffered and live events with SSE headers and cleans up disconnects', async () => {
    ...
    const response = await app.inject({
      url: `/api/demo/events?layer=web&gameId=${gameId}`,
  ...
  it('defaults to loopback configuration and rejects arbitrary interface addresses', () => {
    ...
    expect(() =>
      parseBackendConfig({ HOST: '192.168.1.10', DATABASE_PATH: ':memory:' }),
    ).toThrow();
  });
});
