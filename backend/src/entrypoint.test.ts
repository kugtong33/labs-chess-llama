...
beforeAll(async () => {
  fixtureRoot = await mkdtemp(
    join(tmpdir(), 'chess-llama-backend-entrypoint-'),
  );
  fixtureBackendRoot = join(fixtureRoot, 'backend');
  builtDirectory = join(fixtureBackendRoot, 'dist');
  ...
  await symlink(
    join(fixtureBackendRoot, 'src'),
    join(fixtureRoot, 'source-current'),
  );
  await symlink('backend/dist', join(fixtureRoot, 'current'));
  ...
  for (const directory of fixturePackages) {
    ...
    await executeFile(
      ...
      [
        ...
        directory === 'backend' ? 'src/main.ts' : 'src/index.ts',
        '--format',
        'esm',
      ],
      { cwd: packageRoot },
...
describe.each(['source', 'built'] as const)('%s backend entrypoint', (kind) => {
  function paths() {
    return kind === 'source'
      ? {
          direct: join(fixtureBackendRoot, 'src/main.ts'),
  ...
  it.each(['direct', 'linked'] as const)(
    ...
    async (mode) => {
      ...
      // Missing required config proves startBackend ran without opening a server.
  ...
  it('can be imported through a symlink without starting the backend', async () => {
    const entrypoint = paths();
